    ...
    <Card highlight={hl} className="relative h-full overflow-hidden">
      {c.img && (
        <div aria-hidden="true" className="pointer-events-none absolute inset-0">
          <img src={c.img} alt="" loading="lazy" className="h-full w-full object-cover opacity-[0.09]" />
          <div className="absolute inset-0 bg-gradient-to-t from-white via-white/85 to-white/45"></div>
        </div>
      )}
      <div className={'relative inline-flex ' + (hl ? 'rounded-lg bg-white/15 p-2 text-white' : 'text-primary')}>
        <Icon name={c.icon} className="h-6 w-6" />
      </div>
      <div className="relative mt-6">
        <h3 className={'text-[18px] font-semibold ' + (hl ? 'text-white' : 'text-ink')}>{c.title}</h3>
        <p className={'mt-2 text-[14.5px] leading-[1.5] ' + (hl ? 'text-white/80' : 'text-mute')}>{c.text}</p>
        {c.img && (
          <span className="mt-3 inline-block font-mono text-[10px] uppercase tracking-[0.14em] text-faint">
  ...
  return (
    <section className="bg-tint py-24 lg:py-[120px]">
      <Shell>
        ...
        <Reveal delay={120}>
          <p className="headline mx-auto mt-14 max-w-2xl text-center text-[24px] font-semibold leading-[1.3] text-ink sm:text-[30px]">
...
const FAQS = [
  ...
  },
  {
    ...
    a: 'Não. A migração acontece em etapas, com o portal operando normalmente durante todo o processo. Cada transição tem janelas controladas e plano de rollback disponível. Para uma operação editorial, downtime não é aceitável — e o nosso processo é construído com essa premissa.',
  },
  {
  ...
  },
  {
    ...
    a: 'Não é um sistema de tickets. É uma equipe que conhece a sua operação e o que está em jogo. O objetivo é que você nunca precise lembrar que tem suporte — porque as coisas simplesmente funcionam.',
  },
]
